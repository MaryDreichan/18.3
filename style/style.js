const data = [ //массив с данными о супергероях
  { 
  "name": "Бетмен",
  "universe": "DC Comics",
  "alterego": "Брюс Уэйн",
...
function createCard({name, universe, alterego, occupation, friends, superpowers, url, info, raiting, id }) { 
  ...
  const template = `
    <div>
      ...
      <div class="wr_rating">
      <input type="number" value="${raiting}" />
...
container.addEventListener('click', (event) => {
  if (event.target.classList.contains('addRaiting')) {
    const input = event.target.previousElementSibling;
    const id = event.target.id;
    const value = parseInt(input.value);
    saveRaiting(id, value);
  }
});

function saveRaiting(id, value) {
  ...
  localStorage.setItem('data', JSON.stringify(newResponse));
}